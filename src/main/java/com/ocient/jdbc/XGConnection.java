...
public class XGConnection implements Connection
{
	...
	/**
	 * Equals and hashcode should almost always be autogenerated. Only roll your own if
	 * 1. have a legitimate reason to do so (very rare)
	 * 2. know what you're doing
	 * 3. write unit tests for  object equality
	 */
	...
	public boolean equals(Object o) {
		...
		XGConnection xGConnection = (XGConnection) o;
		return Objects.equals(originalIp, xGConnection.originalIp) && 
							originalPort == xGConnection.originalPort && 
							Objects.equals(user, xGConnection.user) && 
							Objects.equals(database, xGConnection.database) && 
							networkTimeout == xGConnection.networkTimeout && 
							Objects.equals(tls, xGConnection.tls) && Objects.equals(pwd, xGConnection.pwd) && 
							Objects.equals(properties, xGConnection.properties) && Objects.equals(setSchema, xGConnection.setSchema) && 
							setPso == xGConnection.setPso && Objects.equals(maxRows, xGConnection.maxRows) && 
							Objects.equals(maxTime, xGConnection.maxTime) && 
	...
	/**
	 * Turn off connection caching with SSO handshake flow. These sorts of connections cannot be reasonably cached.
	 * The log in proceess involves the user going to an external url and authenticating. If a "cached" connection 
	 * of this is returned, then it is not really connected and will trigger a reconnect, which will cause us to
	 * look in the cache for this connection. Remember, we take 30 seconds to return connections to the cache.
	 * If it's not there, then we need the user to use another url. Overall, this is too much to try to get working
	 * for the minimal benefits of connection caching this one special type of connection that requires user input.
	 * Let's disable this for now since we are getting rid of this pooling in the connector's rewrite.
	 */	
	boolean isDisabledConnCaching(){
		final XGConnection.HandshakeType handshake = XGConnection.HandshakeType.valueOf((properties.getProperty("handshake", "GCM").toUpperCase()));
		return handshake == HandshakeType.SSO && user.toLowerCase().equals("") && pwd.toLowerCase().equals("");
	}
}
